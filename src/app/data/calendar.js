...
export default class DataCalendar {
  ...
  LoadCalendars () {
    ...
    for (var k in Config.Calendars) {
      ...
      var timestamp = this.starttime
        ? '/' + parseInt(this.starttime.getTime() / 1000)
        : ''
  ...
  OnData (events) {
    ...
    events.forEach(e => {
      ...
      if (old) {
        if (e.updated !== old.updated) this.events[e.id] = e
      } else this.events[e.id] = e
    ...
    this._timer = setTimeout(() => {
      StatusInfo.Display('Next Update: ', true)
      var timedout = new Date().getTime() - 10000

      for (var k in this.events) {
        var e = this.events[k]
        if (e._touched < timedout) delete this.events[e.id]
      }
